    ...
    <div class="kt-portlet__head kt-portlet__head--lg">
      <div class="kt-portlet__head-label">
        ...
        <h3 class="kt-portlet__head-title">
          Detail Perizinan Pendidikan - Tim Teknis
        </h3>
      </div>
...
function reason(key, head) {
  ...
  $("#modal-reason").modal("show");
          ...
          $('#post-reason').submit(function(e){ // tolak kolom
            const route= "{{ route('reason.teknis', ['id' => $data->pendidikan->id, 'jenis' => 'pendidikan']) }}";
            e.preventDefault();
    ...
    function ceklis(key, head) { // ceklis status kolom
      $(document).on('click', '#ceklis', function(){
        Swal.fire({
          ...
          if (result.value) {
            var me = $(this),
            url = "{{ route('ceklis.teknis', ['id' => $data->pendidikan->id, 'jenis' => 'pendidikan']) }}",
            token = $('meta[name="csrf-token"]').attr('content');
            $.ajax({
              url: url,
              method: "POST",
